...
function matchFromStoredProfile(job: JobMatchInput) {
  return computeJobMatch(getUserProfileForMatch(), job);
}
...
export const jobApi = {
  ...
  getJobs: async (params?: {
    ...
    return {
      records: paginatedJobs.map((job, index) => {
        ...
        if (job.job_title) {
          const industry =
            companiesData.find((c) => c.company_name === job.company_name)?.industry ||
            "IT/互联网";
          const salaryMin = Math.round(job.salary_min / 1000);
          const salaryMax = Math.round(job.salary_max / 1000);
          const m = matchFromStoredProfile({
            title: job.job_title,
            city: job.city,
            industry,
  ...
  getJobDetail: async (id: string) => {
    ...
    if (job) {
      ...
      const industry = job.industry || company?.industry || 'IT/互联网';
      const match = matchFromStoredProfile({
        title: job.title,
        city: job.city,
        industry,
        company: job.company,
      ...
      return {
        ...
        matchDimensions: match.matchDimensions,
        recommendReasons: match.recommendReasons,
      };
    }
    ...
    const industry = company?.industry || 'IT/互联网';
    const match = matchFromStoredProfile({
      ...
      responsibilities: [crawledJob.responsibilities],
    });
  ...
  getAllJobs: async (params?: {
    ...
    const crawledJobs = jobsData.map((job, index) => {
      ...
      return {
        ...
        companyType:
          companiesData.find((c) => c.company_name === job.company_name)?.company_type ||
          "私企",
        city: job.city,
        salaryMin,
        salaryMax,
        ...
        responsibilities: [job.responsibilities],
        requirements: [job.requirements],
        recommendReasons: m.recommendReasons,
